...
const EditTask = () => {
    ...
    const getTask = async () => {
        setIsLoading(true)
        try {
            const res = await API.get(`task-management/task/task-by-id/${task_id}`);
    ...
    const handleEditTask = async (e) => {
        ...
        try {
            const res = await API.put(`/task-management/task/update-task/${task_id}`, {
    ...
    return (
        <div className="edit-task-wrapper">
            <h3 className="mx-3 my-3">Edit Task</h3>
            <div className="create-task-form-wrapper">
                <form className='create-task-form' onSubmit={handleEditTask}>
                    {
                        isLoading ? (
